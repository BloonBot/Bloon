    ...
    public class StreamEvents
    {
        ...
        public async void Register()
        {
            if (this.monitor == null)
            {
                ...
                List<string> channelIDs = (await this.twitchAPI.Helix.Users.GetUsersAsync(logins: new List<string> { this.twitchChannel }).ConfigureAwait(false))
                    .Users
                    .Select(x => x.Id)
                    .ToList();
        ...
        private async void OnStreamOffline(object sender, OnStreamOfflineArgs args) => await this.activityManager.ClearStreamAsync().ConfigureAwait(false);
        ...
        {
            Channel channel = await this.twitchAPI.V5.Channels.GetChannelByIDAsync(channelID).ConfigureAwait(false);
            await this.activityManager.SetStreamAsync(1, stream.Title, channel.Url, true).ConfigureAwait(false);
        }
    }
}
